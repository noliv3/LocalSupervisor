...
if ($logsRoot === null) {
    sv_log_system_error($config, 'media_service_log_root_unavailable', ['error' => $logsError]);
...
$limit = 50;
$sleepMs = (int)($config['workers']['media']['sleep_ms'] ?? 1500);
if ($sleepMs < 100) {
    $sleepMs = 100;
}
...
while ($running) {
    ...
    try {
        ...
        sv_log_system_error($config, 'media_service_batch_failed', ['worker_type' => $serviceName, 'error_code' => 'batch_failed', 'error' => $e->getMessage()]);
