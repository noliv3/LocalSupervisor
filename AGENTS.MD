...
- Forge-HTTP-Aufrufe nutzen die WebUI-API-Endpunkte `/sdapi/v1/options` (Healthcheck), `/sdapi/v1/txt2img` bzw. `/sdapi/v1/img2img` (Payload), optional `/sdapi/v1/progress` und verzichten auf Token-Auth; Basic Auth nur, wenn `forge.basic_auth_user/pass` gesetzt sind. Job-Logs enthalten Ziel-URL (ohne Credentials), HTTP-Status und ein 200-Zeichen-Snippet.
