...
- Öffentlicher Ollama-Webpfad (`WWW/ollama.php`) darf für `action=run` kein Success signalisieren; stattdessen `blocked/disabled_in_web_path`.
- Admin-UI-Flows für Ollama (Dashboard/Media-Analyze) sollen den internen Endpoint (`WWW/internal_ollama.php`) nutzen; `action=run` ist dort Alias für `run_once` mit Spawn-Metadaten.
...
- Fehlende Systemtools (`ffmpeg`, `ffprobe`, `magick`) sind kritische Blocker für Medienverarbeitung/Thumbnails (siehe Doctor); bei fehlendem `ffmpeg`/`ffprobe` endet Doctor mit Exit-Code `20`.
- Fehlende PHP-GD-Erweiterung (`imagecreatetruecolor`) ist ein harter Blocker für `selftest_cli.php` (`GD nicht verfügbar`).
- Fehlende Vidax-Konfigurationen oder Asset-Manifests sind kritische Doctor-Fehler und führen ebenfalls zu Exit-Code `20`.
...
- Ollama-Funktionen benötigen erreichbaren lokalen Dienst plus geladene, passende Modelle (z. B. `llava`, `llama3`, `nomic-embed-text`).
- Schreibrechte auf `LOG_PATH` und DB-Verzeichnis (insb. SQLite) sind zwingend für Logging/Persistenz.

## 9) Dashboard-Loganalyse (V1)

- Dashboard enthält eine Incident-orientierte Loganalyse (Top-10 priorisiert) statt Rohloganzeige.
- API dafür: `WWW/api/logs/incidents.php` mit internem Zugriffsschutz.
- Analyse läuft batch-orientiert pro Tag, ohne LLM-Aufrufe im Request-Path.
- Heuristik: Jobs ohne valides Ergebnis gelten als `completed_no_result` und nicht als success.
- Rohlogs bleiben unverändert erhalten; UI nutzt nur aggregierte Gruppen + Beispielzeilen.
