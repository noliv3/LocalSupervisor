...
- **Rescan**: Sendet vorhandene Medien erneut an den Scanner, aktualisiert Status/Ratings/NSFW und füllt fehlende Metadaten nach; Single-Media-Rescan läuft als Job (`rescan_media`) und nutzt dieselbe Queue wie der Scan-Worker. Jeder Lauf persistiert `scan_results.run_at` und ersetzt `media_tags` für `locked=0`; Fehler landen im Jobstatus + Audit, erfolgreiche Läufe räumen `scan_stale` ab.
