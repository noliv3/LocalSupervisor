...
- `SCRIPTS/` – CLI-Tools und gemeinsame Scan-Logik (`scan_core.php`); `operations.php` bündelt die Web-/CLI-Einstiegspunkte für Scan/Rescan/Filesync/Prompt-Rebuild/Consistency-Check.【F:SCRIPTS/operations.php†L1-L235】【F:SCRIPTS/scan_core.php†L4-L118】
...
- **Dashboard-Reichweite**: Das Web-Dashboard startet Scan-, Rescan-, Filesync-, Prompt-Rebuild- und Consistency-Check-Läufe direkt über die gemeinsame Operations-Schicht; Backup/Migration bleiben separat manuell.【F:WWW/index.php†L5-L136】【F:SCRIPTS/operations.php†L1-L235】
...
- **Produktiv**: Dünne CLI-Wrapper (`scan_path_cli.php`, `rescan_cli.php`, `filesync_cli.php`, `prompts_rebuild_cli.php`, `consistency_check.php`) delegieren an `operations.php` und spiegeln die Web-Einstiegspunkte; daneben bestehen `cleanup_missing_cli.php`, `migrate.php`, `db_backup.php` und `init_db.php` als direkte Maintenance-Tools.【F:SCRIPTS/operations.php†L1-L235】【F:SCRIPTS/scan_path_cli.php†L4-L67】【F:SCRIPTS/rescan_cli.php†L4-L49】【F:SCRIPTS/filesync_cli.php†L4-L49】【F:SCRIPTS/prompts_rebuild_cli.php†L4-L49】【F:SCRIPTS/consistency_check.php†L4-L46】【F:SCRIPTS/cleanup_missing_cli.php†L4-L98】【F:SCRIPTS/migrate.php†L1-L173】【F:SCRIPTS/db_backup.php†L1-L118】【F:SCRIPTS/init_db.php†L1-L79】
...
- Logs liegen nach Typ unter `LOGS/<channel>/` (Scan, Rescan, Filesync, Prompts, Consistency, Security) mit Dateischema `<channel>_YYYYMMDD_HHMMSS.log` (Security täglich, übrige pro Lauf) und automatischer Rotation (operativ 50 Dateien, Consistency 30, Security 60).【F:SCRIPTS/logging.php†L7-L65】【F:WWW/index.php†L49-L181】

## Weboberfläche
Das Dashboard (`WWW/index.php`) stellt eine einfache Übersicht bereit: PDO-Verbindung über `CONFIG/config.php`, Ausgabe der vorhandenen DB-Tabellen (SQLite) sowie Formulare, die Scan/Rescan/Filesync, Prompt-Rebuild und Consistency-Checks direkt über die zentrale Operations-Schicht starten und deren Logs anzeigen.【F:WWW/index.php†L5-L194】【F:SCRIPTS/operations.php†L1-L308】
Unterhalb der Formulare zeigt das Dashboard eine komprimierte Statistik über Medien (Typ/Status/Rating/NSFW), Prompts, Tags, Scan-Resultate, media_meta sowie Import-/Job-Logs und listet eine CLI-Referenz mit Kategorien und Beispielaufrufen für die wichtigsten Skripte.【F:WWW/index.php†L196-L502】

- `mediadb.php`: Listenansicht mit Filtern nach Typ (Bild/Video), Prompt-Flag, Metadaten-Flag, Status, Mindest-Rating und Pfad-Substring; NSFW lässt sich über `adult=1` einblenden. Zeigt Typ-Badges, Prompt-/Metadaten-Indikatoren und Links zur Detailseite/Originalstream.【F:WWW/mediadb.php†L1-L377】【F:WWW/media_stream.php†L1-L146】
  Die Web-Ansichten greifen direkt auf die in `paths.*` konfigurierten Medienverzeichnisse zu; `media_stream.php` validiert Pfade gegen SFW/NSFW-Basisverzeichnisse und liefert Dateien ohne Webroot-Symlinks aus.【F:SCRIPTS/paths.php†L1-L42】【F:WWW/media_stream.php†L1-L146】
...
- Web-Schreibzugriffe auf Scanner/Filesync/Rescan oder spätere Mutationen verlangen den `internal_api_key` (Header `X-Internal-Key`, Query/Cookie `internal_key`) **und** eine Treffer-IP in `ip_whitelist`; andernfalls antworten geschützte Endpunkte mit HTTP 403 und schreiben einen Security-Log.【F:SCRIPTS/security.php†L14-L79】【F:CONFIG/config.php†L41-L52】
- `sv_require_internal_access` bündelt die Prüfungen und wird an kritischen Web-Einstiegspunkten wie dem Dashboard verwendet.【F:WWW/index.php†L5-L184】【F:SCRIPTS/security.php†L14-L79】
