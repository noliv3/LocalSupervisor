...
- **Mode- & Override-Flow**: `sv_decide_forge_mode()` erzwingt standardmäßig `img2img` (Denoise 0.25) und wechselt nur bei `_sv_force_txt2img` oder ungeeignetem Input auf `txt2img`. Die Override-Keys `_sv_manual_prompt`, `_sv_manual_negative`, `_sv_negative_allow_empty`, `_sv_seed`, `_sv_steps`, `_sv_denoise`, `_sv_sampler`, `_sv_scheduler`, `_sv_model` landen unverändert in `forge_request_json`; Responses spiegeln `decided_mode/reason/denoise` und die genutzten Quellen/Seeds/Sampler wider.
