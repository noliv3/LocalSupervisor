...
- **Automatische Prompt-Normalisierung**: `sv_extract_metadata` sammelt EXIF-Kommentare (UserComment/ImageDescription/XpComment/Comment) und PNG-Textfelder wie `parameters`, `prompt`/`Negative prompt`, `sd-metadata` (Forge) oder `workflow` (Comfy) und überführt sie über `SCRIPTS/prompt_parser.php` in `prompts`, ohne bestehende Felder zu überschreiben.【F:SCRIPTS/scan_core.php†L478-L556】【F:SCRIPTS/prompt_parser.php†L1-L220】 Altbestände lassen sich bei Bedarf via `php SCRIPTS/prompts_rebuild_cli.php --limit=250 --offset=0` nachziehen; `exif_prompts_cli.php` dient nur noch als Legacy-Helfer für Spezialfälle.【F:SCRIPTS/prompts_rebuild_cli.php†L1-L96】【F:SCRIPTS/exif_prompts_cli.php†L1-L80】
