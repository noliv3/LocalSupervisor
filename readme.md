...
## Forge-Regeneration
- **Ablauf**: In der Detailansicht (`media_view.php`) kann für Bildmedien mit vollständigen Prompts die Aktion „Regen über Forge“ ausgelöst werden. Vor dem Call wird der Prompt bewertet (A/B/C-Heuristik); schwache Prompts erhalten Tag-Ergänzungen, kritische Prompts werden durch einen Tag-basierten Prompt ersetzt oder abgewiesen. Das Forge-Payload nutzt den modellierten Prompt, prüft das Modell gegen die lokal verfügbaren Varianten und greift bei Bedarf auf den Fallback `SDXL_FP16_waiNSFWIllustrious_v120.safetensors` zurück. Der Forge-Job wird synchron ausgeführt.
- **Replace in place**: Das gelieferte Bild wird als Backup gesichert und anschließend auf demselben Pfad geschrieben (Hash/Größe/Auflösung werden aktualisiert). Danach läuft ein gezielter Re-Scan (NSFW/Tags) sowie eine Metadaten- und Prompt-Aktualisierung, sodass Tags/Prompts/Meta zum neuen Bild passen. Request-/Response-Daten bleiben im `jobs`-Eintrag auditierbar.
