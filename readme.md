...
## Forge-Regeneration
- **Ablauf**: In der Detailansicht (`media_view.php`) kann für Bildmedien mit vollständigen Prompts die Aktion „Regen über Forge“ ausgelöst werden. Dabei wird ein Forge-Payload aus den gespeicherten Prompt-Parametern gebaut, das gewünschte Modell vor dem Versand gegen die lokal verfügbaren Forge-Modelle geprüft und bei Bedarf auf den Fallback `SDXL_FP16_waiNSFWIllustrious_v120.safetensors` gemappt. Ein Job in `jobs` mit Typ `forge_regen` wird angelegt und optional sofort an den konfigurierten Forge-Endpunkt gesendet.
...
- **Unveränderte Ablage**: Die physische Ablage und Hash-Logik bleiben unverändert; Forge-Ausgaben werden derzeit nur als Jobs erfasst und nicht automatisch in den Medienbestand zurückgeschrieben. Modellnamen im Prompt und im lokalen Forge können abweichen; die Zuordnung erfolgt best effort, nutzt aber bei Unsicherheit den definierten Fallback.
